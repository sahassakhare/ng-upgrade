...
export class JestToVitestTransformer implements TransformationHandler {
    readonly type = 'config';

    async apply(projectPath: string, change: BreakingChange): Promise<void> {
        console.log(`Applying Jest-to-Vitest test migration: ${change.description}`);

        // 1. Remove Jest Dependencies
        await this.removeJestDependencies(projectPath);
        ...
        // 5. Remove Jest specific artifacts
        await this.removeJestArtifacts(projectPath);

        // 6. Update TSConfig types
        await this.updateTsConfigs(projectPath);

        // 7. Update Test Files Regex (jest.fn -> vi.fn)
        await this.updateTestFiles(projectPath);
    }

    private async removeJestDependencies(projectPath: string): Promise<void> {
        ...
        try {
            ...
            if (removed) {
                ...
                console.log('✓ Removed Jest dependencies');
            }
        } catch (e) {
            console.warn('Failed to remove Jest dependencies', e);
        }
    }
        ...
        try {
            ...
            for (const projectName of Object.keys(projects)) {
                ...
                if (project.architect && project.architect.test) {
                    if (project.architect.test.builder.includes('jest')) {
            ...
            if (updated) {
                ...
                console.log('✓ Updated angular.json to remove Jest targets (replaced by npm test)');
            }
        } catch (e) { }
    }
...
export default defineConfig({
  ...
  test: {
    ...
    private async removeJestArtifacts(projectPath: string): Promise<void> {
        const findArtifactFiles = async (dir: string): Promise<string[]> => {
            ...
            for (const file of list) {
                ...
                } else if ((file.startsWith('jest.') && file.endsWith('.js')) || file === 'jest.config.ts' || file === 'setup-jest.ts') {
        ...
        try {
            ...
            console.warn('Failed to clean up all Jest config artifacts', e);
        }
    }
        ...
        try {
            ...
            for (const file of tsConfigFiles) {
                ...
                if (content.includes('"jest"')) {
                    content = content.replace(/"jest"/g, '"vitest/globals"');
            ...
            if (updatedCount > 0) {
                console.log(`✓ Updated ${updatedCount} tsconfig files (replaced jest types with vitest/globals)`);
            }
        } catch (e) {
    ...
    private async updateTestFiles(projectPath: string): Promise<void> {
        ...
        try {
            ...
            for (const file of specFiles) {
                ...
                if (content.match(/jest\.mock\(|jest\.requireActual\(|jest\.useFakeTimers\(/)) {
            ...
            if (updatedCount > 0) {
                console.log(`✓ Updated ${updatedCount} test files from Jest API to Vitest API`);
            }
            if (warningFiles.length > 0) {
                console.warn(`⚠ WARNING: Found ${warningFiles.length} files with complex Jest AST structures (e.g., jest.mock). Manual developer review required for Vitest equivalents:`);
