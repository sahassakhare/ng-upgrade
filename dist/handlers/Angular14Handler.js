...
class Angular14Handler extends BaseVersionHandler_1.BaseVersionHandler {
    ...
    getBreakingChanges() {
        return [
            ...
            this.createBreakingChange('ng14-standalone-components', 'api', 'low', 'Standalone components introduced', 'New way to create components without NgModules - completely optional', 'Standalone components are opt-in. Existing NgModule approach continues to work'),
            // TypeScript version requirement
            this.createBreakingChange('ng14-typescript-version', 'dependency', 'medium', 'TypeScript 4.7+ required', 'Angular 14 requires TypeScript 4.7.2 or higher', 'Update TypeScript to version 4.7.2 or higher'),
            // Node.js version requirement
            this.createBreakingChange('ng14-nodejs-version', 'dependency', 'medium', 'Node.js 14.15+ required', 'Angular 14 requires Node.js 14.15.0 or higher', 'Update Node.js to version 14.15.0 or higher'),
            // Optional injectors
            this.createBreakingChange('ng14-optional-injectors', 'api', 'low', 'Optional injectors and inject() function', 'New inject() function for dependency injection outside constructors', 'New feature - existing constructor injection continues to work'),
            // Protected route guards
            this.createBreakingChange('ng14-protected-guards', 'api', 'low', 'Protected route guards introduced', 'New guard type for protecting routes from unauthenticated access', 'New feature - existing guards continue to work'),
            // Strict typed forms
            this.createBreakingChange('ng14-typed-forms', 'api', 'low', 'Strict typed reactive forms', 'Enhanced type safety for reactive forms', 'Opt-in feature - existing forms continue to work')
        ];
    }
     ...
     */
    async setupStandaloneComponentsSupport(projectPath) {
...
`;
     ...
     */
    async implementOptionalInjectors(projectPath) {
        try {
            const exampleDir = path.join(projectPath, 'src/app/examples');
            await fs.ensureDir(exampleDir);
...
`;
     ...
     */
    async setupProtectedRouteGuards(projectPath) {
        try {
...
`;
     ...
     */
    async configureExtendedDevAPIs(projectPath) {
     ...
     */
    async setupCLIAutoCompletion(projectPath) {
        try {
     ...
     */
    async configureStrictTypedForms(projectPath) {
        try {
            const exampleDir = path.join(projectPath, 'src/app/examples');
            await fs.ensureDir(exampleDir);
...
`;
     ...
     */
    async implementPageTitleStrategy(projectPath) {
        try {
...
`;
     ...
     */
    async updateTsConfigForStandalone(projectPath) {
