...
class FileContentPreserver {
     ...
     */
    static async updateMainTsFile(filePath, targetAngularVersion) {
     ...
     */
    static async updateComponentFile(filePath, transformations) {
        if (!await fs.pathExists(filePath)) {
            return;
        }
     ...
     */
    static async updateTemplateFile(filePath, targetAngularVersion) {
        if (!await fs.pathExists(filePath)) {
            return;
        }
        let content = await fs.readFile(filePath, 'utf-8');
     ...
     */
    static extractCustomProviders(sourceFile) {
        const providers = [];
     ...
     */
    static extractCustomImports(sourceFile) {
        const imports = [];
     ...
     */
    static extractAppModulePath(sourceFile) {
...
`;
     ...
     */
    static async applyTransformation(content, transform) {
     ...
     */
    static findLastImportIndex(content) {
     ...
     */
    static updateImport(content, oldImport, newImport) {
     ...
     */
    static migrateControlFlowSyntax(content) {
        ...
        content = content.replace(/<(\w+)\s+\*ngIf="([^"]+)"\s*>/g, '@if ($2) {\n  <$1>');
        // Close the @if block
        content = content.replace(/<\/(\w+)>(\s*)<!--\s*end\s*ngIf\s*-->/g, '</$1>\n}');
        return content;
    }
}
exports.FileContentPreserver = FileContentPreserver;
//# sourceMappingURL=FileContentPreserver.js.map